...
export default function ProjectNavSmall() {
  ...
  return (
    <Flex className="md:hidden gap-sm p-[2px] mb-md">
      <DialogSquare variant="outline" type="smallNavigation" hideText={true} triggerClass={cn(smallNavigationClass, 'icon-xxs p-0 size-xl mr-auto sm:mr-0')} />
      ...
      <Button
        ...
        <select
          ...
          defaultValue={""}
        >
          <option value="" disabled>Shared projects</option>
          {projects?.collaborations?.map((project: Collaborator) => (
            <option key={project.project.id} value={`/project/${project.project.id}`}>
