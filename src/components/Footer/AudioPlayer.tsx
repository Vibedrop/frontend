...
export default function AudioPlayer() {
  ...
  useEffect(() => {
    ...
    const updateSongDuration = () => setDuration(audioRef.current!.duration);
    ...
    return () => {
      ...
      audioRef.current?.removeEventListener("loadedmetadata", updateSongDuration);
  ...
  return (
    <Flex className={`flex-row flex-wrap md:flex-nowrap items-center gap-xs md:gap-xxl justify-between px-0 py-md lg:px-xl lg:py-lg lg:pt-md ${!currentSong ? "opacity-50 pointer-events-none" : "" }`}
    >
        ...
        {currentSong && (
          <Flex className={`flex w-full md:max-w-[420px] items-center gap-md ${!currentSong ? "hidden" : ""}`}>
            ...
            <Flex className="flex-row max-w-full md:flex-col gap-xs md:gap-0 content-center items-start m-auto lg:m-0">
              ...
              {/* For small screens */}
              <Text truncate className="text-body-s xl:text-body-l text-muted md:hidden">{currentSong.name}</Text>
            </Flex>
          </Flex>
        )}
      </Flex>
      ...
      <Flex className="flex-col w-full md:w-[36%] xl:w-[50%] max-w-[1080px] grow gap-md lg:py-md items-center">
        ...
        <Flex direction="row" className="w-full max-w-[580px] items-center justify-around sm:justify-between">
      ...
      <Flex className="w-full md:w-[32%] xl:w-[20%] items-center place-content-end hidden md:flex">
        <Flex className={`flex w-full max-w-[280px] xl:max-w-[380px] items-center gap-sm mt-4 ${!currentSong ? "hidden" : ""}`}>
          <button onClick={toggleMute}>
            {audioRef.current?.muted ? <VolumeX className="icon-md" /> : <Volume2 className="icon-md" />}
          </button>
