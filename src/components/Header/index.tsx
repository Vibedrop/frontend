...
export default function Header() {
  ...
  return (
    <header className="flex justify-between items-center my-md lg:my-lg">
      <Link href={logoHref} className="flex items-center gap-md lg:gap-xl logo hover:opacity-80 transition-all duration-200">
      ...
      {isStart && !isAuthenticated && (
        <Flex className="gap-sm items-center">
          <Button asChild variant="outline" className="lg:w-[100px] max-h-[36px] lg:max-h-none">
            <Link href="/sign-in" >
              <Text className="text-header-s">Sign in</Text>
            </Link>
          </Button>

          <Button asChild  className="lg:w-[100px] max-h-[36px] lg:max-h-none">
            <Link href="/sign-up">
